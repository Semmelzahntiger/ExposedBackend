...
# ExposedBackend
Status: in active development; Backend is feature functional but not polished; Frontend is currently a prototype to test the backend.
...
## Technical Overview
* Backend runs on Java 25
* Authentication: REST-based registration and login with Spring Security. Authentication using JWT access tokens and refresh-tokens. Protected endpoints are secured through a filter chain.
* Data ingestion: Users upload their platform specific data export as a ZIP file. The backend will decode the file and forward the contents to a platform-specific parser (parser is resolved via dependency injection) that normalizes the data for database storage
* Persistence: parsed data is stored in a PostgreSQL database via Spring Data JPA with modeled relations and indexed queries
* Real-time gameplay: clients connect over WebSocket (Raw Websocket with custom JSON communication protocol) and authenticate with their access token. Users can create a room or join an existing one using the autogenerated 6 digit code of each room.
* Extensibility: Adding new platforms primarily requires just implementing a new parser service (Currently WIP)
